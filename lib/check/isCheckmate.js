System.register(["check", "positions", "constants/move-vectors"], function (exports_1, context_1) {
    "use strict";
    ...
    var check_1, positions_1, move_vectors_1;
    ...
    function isCheckmate(board, defender, boardAnnotations) {
        ...
        for (var i = 0; i < move_vectors_1.kingVectors.length; i++) {
            ...
            var kingMovesTo = positions_1.displaceTo(kingPosition, vector);
            if (positions_1.isOnBoard(kingMovesTo) && positions_1.playerAt(board, kingMovesTo) !== defender) {
                if (!check_1.movesIntoCheck(board, kingPosition, kingMovesTo, boardAnnotations)) {
                    return false;
                }
            }
        }
        var attackLines = check_1.generateLinesOfAttack(board, defender, kingPosition);
        ...
        var attacker = positions_1.otherPlayer(defender);
        try {
            for (var _b = __values(checkLine.value), _c = _b.next(); !_c.done; _c = _b.next()) {
                var positionOnCheckLine = _c.value;
                var defensiveMoves = check_1.generateLinesOfAttack(board, attacker, positionOnCheckLine);
                ...
                while (!defensiveMoveInfo.done) {
                    ...
                    if (!check_1.movesIntoCheck(board, defendingPieceMovesFrom, positionOnCheckLine, boardAnnotations)) {
                        return false;
                    }
