    ...
    return {
        ...
        execute: function () {
            isOkCastle = function (board, kingFrom, kingTo, boardAnnotations) {
                ...
                if (player === "White") {
                    if (!whiteKingSideCastlingPrecluded && index_1.areSamePositions(kingTo, position_1.Position.G1)) {
                    }
                    if (!whiteQueenSideCastlingPrecluded && index_1.areSamePositions(kingTo, position_1.Position.C1)) {
                    }
                }
                else if (player === "Black") {
                    if (!blackKingSideCastlingPrecluded && index_1.areSamePositions(kingTo, position_1.Position.G8)) {
                    }
                    if (!blackQueenSideCastlingPrecluded && index_1.areSamePositions(kingTo, position_1.Position.C8)) {
                    }
                }
            };
