System.register(["position-utils/index", "constants/move-vectors", "constants/position", "check/movesIntoCheck"], function (exports_1, context_1) {
    ...
    function kingMoves(board, kingFrom, castlingPreclusions) {
        ...
        var mayCastle = function (isPrecluded, kingTo, castleMovesTo) {
            return !isPrecluded
        ...
        if (player === "White") {
            if (mayCastle(castlingPreclusions.kingSide, position_1.Position.G1, position_1.Position.F1)) {
                legalMoves.add("G1");
            }
            if (mayCastle(castlingPreclusions.queenSide, position_1.Position.C1, position_1.Position.D1)) {
                legalMoves.add("C1");
            }
        }
        else if (player === "Black") {
            if (mayCastle(castlingPreclusions.kingSide, position_1.Position.G8, position_1.Position.F8)) {
                legalMoves.add("G8");
            }
            if (mayCastle(castlingPreclusions.queenSide, position_1.Position.C8, position_1.Position.D8)) {
                legalMoves.add("C8");
            }
        }
        return legalMoves;
