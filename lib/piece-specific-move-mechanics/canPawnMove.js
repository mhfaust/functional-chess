System.register(["position-utils/index", "constants/pieces"], function (exports_1, context_1) {
    ...
    function canPawnMove(board, fromPosition, toPosition) {
        ...
        var forwardDirection = player === pieces_1.Player.White ? 1 : -1;
        ...
        if (stepsSideways === 0) {
            if (index_1.isOccupied(board, toPosition))
                return false;
            if (stepsForward === 2) {
                if (player === pieces_1.Player.Black && index_1.rank(fromPosition) !== 6)
                    return false;
                if (player === pieces_1.Player.White && index_1.rank(fromPosition) !== 2)
                    return false;
            ...
            if (stepsForward !== 1)
                return false;
            if (index_1.isUnOccupied(board, toPosition))
                return false;
